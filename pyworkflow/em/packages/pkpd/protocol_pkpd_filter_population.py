...
from pyworkflow.em.data import PKPDFitting, PKPDSampleFitBootstrap
import numpy as np
import copy
    ...
    def runFilter(self, objId, filterType, condition):
        ...
        for sampleFit in self.population.sampleFits:
            if newSampleFit.parameters == None:
                ...
                newSampleFit.xB = np.empty((0,sampleFit.xB.shape[1]))
                newSampleFit.yB = np.empty((0,sampleFit.xB.shape[1]))
                newSampleFit.R2 = []
                newSampleFit.R2adj = []
                newSampleFit.AIC = []
                newSampleFit.AICc = []
