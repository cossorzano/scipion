...
class ProtPKPDEV0TwoCompartments(ProtPKPDODEBase):
    """ Fit a two-compartmentx model with intravenous absorption to a set of measurements (any arbitrary dosing regimen is allowed)\n
        ...
        The differential equation is D(t)=Amax-Rin*t, V dC/dt = -(Cl+Clp) * C + Clp * Cp + dD/dt, Vp dCp/dt = Clp * C - Clp * Cp\n
        where Rin is the constant input rate, C is the concentration of the central compartment, Cl the clearance, V and
    ...
    def _defineParams(self, form):
        ...
        form.addParam('bounds', params.StringParam, label="Parameter bounds ([tlag], Rin, Cl, V, Clp, Vp)", default="",
                      help="Bounds for time delay, constant input rate, central clearance and volume and peripheral clearance and volume. "\
